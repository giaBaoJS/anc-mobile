...
export const LegendItem = (props: LegendItemProps) => {
  ...
  const centerAlignedCircle = props.legendOffset * 7.1 - CIRCLE_WIDTH / 2;
  // 65% of the legend container height centers the text in relation to the circles
  const centerAlignedText = props.legendOffset * 7.25;
  ...
  return (
    <>
      <Rect
        ...
        ry={8}
        x={
          baseLegendItemX * legendItemNumber -
          (CIRCLE_WIDTH + 3 * textLengthOffset)
        }
      ...
      <Text
        x={
          baseLegendItemX * legendItemNumber +
          (PADDING_LEFT - 3 * textLengthOffset)
        }
        y={centerAlignedText}
        {...props.labelProps}>
        {props.legendText}
