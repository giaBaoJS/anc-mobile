...
> extends Component<AbstractChartProps & IProps, AbstractChartState & IState> {
    ...
    return [...new Array(count + 1)].map((_, i) => {
      ...
      return (
        <Line
          ...
          x1={paddingRight}
          y1={y}
          x2={width - paddingRight * 0.2} // Cường custom
          y2={y}
  ...
  >) => {
    ...
    return labels.map((label, i) => {
      ...
      const x =
        (((width - paddingRight * ratioPaddingLabelXAxis) / labels.length) * i +
          paddingRight +
          horizontalOffset) *
        fac;
      ...
      (_, i) => {
        return (
          <Line
            key={Math.random()}
            x1={Math.floor(
              ((width - paddingRight * ratioPaddingLabelXAxis) /
                (data.length / yAxisInterval)) *
                i +
                paddingRight,
            )}
            y1={paddingTop} // Cường custom
            x2={Math.floor(
              ((width - paddingRight * ratioPaddingLabelXAxis) /
                (data.length / yAxisInterval)) *
                i +
